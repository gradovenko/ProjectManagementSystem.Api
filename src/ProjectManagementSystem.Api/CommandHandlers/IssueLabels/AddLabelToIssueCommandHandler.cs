...
public sealed class AddLabelToIssueCommandHandler : IRequestHandler<AddLabelToIssueCommand, AddLabelToIssueCommandResultState>
{
    ...
    public AddLabelToIssueCommandHandler(IUserGetter userGetter, ILabelGetter labelGetter,
    ...
    public async Task<AddLabelToIssueCommandResultState> Handle(AddLabelToIssueCommand request, CancellationToken cancellationToken)
    {
        User? user = await _userGetter.Get(request.UserId, cancellationToken);

        if (user == null)
            return AddLabelToIssueCommandResultState.UserNotFound;
        
        Issue? issue = await _issueRepository.Get(request.IssueId, cancellationToken);

        if (issue == null)
            return AddLabelToIssueCommandResultState.IssueNotFound;

        Label? label = await _labelGetter.Get(request.LabelId, cancellationToken);

        if (label == null)
            return AddLabelToIssueCommandResultState.LabelNotFound;

        IssueLabel? issueLabel = await _issueLabelGetter.Get(request.IssueId, request.LabelId, cancellationToken);

        if (issueLabel != null)
            return AddLabelToIssueCommandResultState.IssueUserLabelAlreadyExists;

        issue.AddLabel(user.Id, label.Id);

        await _issueRepository.Save(issue, cancellationToken);

        return AddLabelToIssueCommandResultState.LabelToIssueAdded;
    }
}